let mover = {
    ...
    getNextPosition(direction) {
        while (true) {
            ...
            // Добавлены возможности ходить по диогонали
            switch (direction) {
                case 1:
                    nextPosition.y++;
                    nextPosition.x--;
            ...
            //Проверяем не идет ли игрок в стену и даем возможность ввести новые координаты
            if (nextPosition.x < 0 || nextPosition.y < 0 || nextPosition.x >= config.colsCount || nextPosition.y >= config.rowsCount) {
                direction = parseInt(prompt('Вы уперлись в стену.Введите число (1, 2, 3, 4, 6, 7, 8, 9), куда вы хотите переместиться, "Отмена" для выхода.'));
                continue;
            } else {
                return nextPosition;
            }
